                ...
                <div key={grupo.id} className="min-w-0 rounded-lg border border-rp-border bg-rp-surface/82">
                    ...
                    <div className="border-t border-rp-border px-4 py-3">
                        ...
                        <thead>
                            ...
                            <th className="w-[7%] pb-2 font-bold">#</th>
                            <th className="w-[37%] pb-2 pr-1 font-bold">Pareja</th>
                            <th className="w-[9%] pb-2 font-bold text-center">PJ</th>
                            <th className="w-[9%] pb-2 font-bold text-center">PG</th>
                            <th className="w-[9%] pb-2 font-bold text-center">PP</th>
                            <th className="w-[11%] pb-2 font-bold text-center">Sets</th>
                            <th className="w-[10%] pb-2 font-bold text-center">Jue</th>
                            <th className="w-[8%] pb-2 font-bold text-center">Pts</th>
                            </tr>
                        </thead>
                        <tbody>
                            {grupo.posiciones.map((pos) => (
                            <tr key={pos.id} className="border-t border-rp-border/50 align-top">
                                <td className="py-2 font-bold text-rp-muted">{pos.posicion}</td>
                                <td className="py-2 pr-1 text-rp-text">
                                    <div className="flex flex-col leading-tight">
                                        {pos.parejaNombre.split('/').map((jugador, indice) => (
                                            <span key={indice} className="break-words">{jugador.trim()}</span>
                                        ))}
                                    </div>
                                </td>
                                <td className="py-2 text-center text-rp-muted">{pos.pj}</td>
                                <td className="py-2 text-center text-rp-muted">{pos.pg}</td>
                                <td className="py-2 text-center text-rp-muted">{pos.pp}</td>
                                <td className="py-2 text-center text-rp-muted">{pos.setsGanados}-{pos.setsPerdidos}</td>
                                <td className="py-2 text-center text-rp-muted">{pos.juegosGanados}-{pos.juegosPerdidos}</td>
